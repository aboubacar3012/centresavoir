...
import { Laptop, Languages, Newspaper, BarChart, User, Calendar, Clock, Award, AlertCircle, BookOpen } from "lucide-react";

// Import des composants de tabs
import { InformatiqueTab, LanguesTab, TraductionTab, AutresFormationsTab } from "../tabs";
...
const Formations = () => {
  ...
  const handleFormationClick = (formationType) => {
  ...
  const formationTabs = [
    {
      ...
      description: "Formations bureautiques, maintenance, réseau, programmation et infographie",
    ...
    },
    {
      ...
      description: "Cours d'anglais, français, arabe et allemand pour tous les niveaux",
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Découvrez nos programmes de formation conçus pour vous propulser dans le monde du numérique
          </p>
        </motion.div>
              ...
              >
                <div className="p-4 flex flex-col items-center text-center">
                  <div className={`p-2 rounded-full ${
                    selectedFormation === tab.id ? 'bg-green-200 text-green-600' : 'bg-gray-100 text-gray-600'
                  }`}>
                    {tab.icon}
                  </div>
                  <h3 className="text-base font-bold text-gray-900 mt-3">{tab.title}</h3>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          >
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="relative h-40 w-full rounded-xl overflow-hidden mb-6">
                <Image
                  src={formationTabs.find(tab => tab.id === selectedFormation)?.image || ""}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end">
                  <div className="p-4 text-white">
                    <h3 className="text-2xl font-bold">
                      {formationTabs.find(tab => tab.id === selectedFormation)?.title}
                    </h3>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:w-1/3 flex flex-col">
                ...
                {formationTabs.map((tab) => (
                  <motion.div
                    ...
                    className={`cursor-pointer border-l-4 ${
                      selectedFormation === tab.id ? 'border-green-500 bg-green-50' : 'border-transparent hover:bg-gray-50'
                    }`}
                    onClick={() => handleFormationClick(tab.id)}
                  >
                    <div className="p-4 lg:p-5 flex items-center gap-3">
                      <div className={`p-2 rounded-full ${
                        selectedFormation === tab.id ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'
                      }`}>
                        {tab.icon}
                      </div>
                      <div>
                        <h3 className="text-base lg:text-lg font-bold text-gray-900">{tab.title}</h3>
                        <p className="text-xs lg:text-sm text-gray-600 mt-1 hidden lg:block">{tab.description}</p>
                      </div>
                    </div>
                  </motion.div>
                ))}
              ...
              <div className="mt-6 bg-gradient-to-br from-indigo-600 to-purple-700 text-white rounded-2xl p-5 shadow-lg hidden lg:block">
                ...
                <p className="text-sm text-indigo-100 mb-4">
                  "Notre mission est de vous donner les compétences pratiques dont vous avez besoin pour réussir dans le monde professionnel d'aujourd'hui."
                </p>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="max-w-6xl mx-auto mt-16 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-2xl p-6 lg:p-8 shadow-xl"
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
            ...
            <div>
              <div className="mb-6">
                <h4 className="text-lg lg:text-xl font-semibold mb-3 lg:mb-4">Frais d&apos;inscription</h4>
                <p className="text-base lg:text-lg">50.000 GNF</p>
              </div>

              <div className="mb-6">
                <h4 className="text-lg lg:text-xl font-semibold mb-3 lg:mb-4 flex items-center gap-2">
                  <Calendar className="w-4 h-4 lg:w-5 lg:h-5" />
                  Jours de formation
                </h4>
                <p className="mb-2">Du lundi au vendredi</p>
                <p>Les cours individuels : samedis et dimanches</p>
              </div>
              ...
              <div>
                ...
                <p className="mb-2">Matin : 9h à 11h, 11h à 13h</p>
                <p>Soir : 15h à 17h, 17h à 19h, 19h à 21h</p>
              </div>
            </div>

            {/* Bloc droit */}
            <div className="bg-white/10 backdrop-blur rounded-xl p-5 lg:p-6">
              <h4 className="text-lg lg:text-xl font-semibold mb-3 lg:mb-4 flex items-center gap-2">
                <AlertCircle className="w-4 h-4 lg:w-5 lg:h-5" />
                Notes importantes
              </h4>
              <ul className="space-y-2 lg:space-y-3 list-disc pl-5">
                <li>Réduction de 10% pour deux formations différentes</li>
                <li>Le paiement mensuel se fait au début de chaque mois</li>
                <li>Les formations linguistiques sont payées au mois</li>
                <li>Une attestation de formation est délivrée à la fin</li>
              </ul>

              <div className="mt-5 lg:mt-6 pt-5 lg:pt-6 border-t border-white/20 flex items-center gap-3">
                <Award className="w-6 h-6 lg:w-8 lg:h-8 text-yellow-300" />
                <p className="font-semibold">Certification officielle remise à la fin de la formation</p>
              </div>
            </div>
          </div>
        </motion.div>
